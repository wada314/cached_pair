...
impl<T, A: Allocator + Clone> CellCollection for VecCollection<T, A> {
    ...
    fn extract_if<F>(self, mut f: F) -> Result<Self::Item, Self>
    where
        F: FnMut(&Self::Item) -> bool,
        Self: Sized,
    {
        // Find the first matching item
        let pos = self.0.borrow().iter().position(|boxed| f(boxed.as_ref()));
...
impl<T: Debug, A: Allocator> Debug for VecCollection<T, A> {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write!(f, "VecCollection({:?})", self.0.borrow())
    }
}
